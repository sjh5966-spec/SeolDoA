...
def ordinary_name(n):
 s=str(n or '').replace(' ','').upper();bad=('스팩','SPAC','리츠','REIT','ETF','ETN')
...
def main():
 ...
 for r,x,sd,fm in base:
  ...
  for h in (5,10,20,60,120):rec[f'R{h}']=pct(float(en.Open),float(path.iloc[h].Close)) if len(path)>h and float(path.iloc[h].Close)>0 else None
  if len(path)>20:rec['MFE20']=float(path.iloc[:21].High.max()/float(en.Open)-1)*100;rec['MAE20']=float(path.iloc[:21].Low.min()/float(en.Open)-1)*100
  rows.append(rec)
 o=pd.DataFrame(rows);o.to_csv(OUT,index=False)
 valid=o[(o.status=='OK')&o.december_fye.eq(True)&o.ordinary_name_candidate.eq(True)&o.R20.notna()&~o.shares_change_gt20pct_R20.astype(bool)].copy() if len(o) else pd.DataFrame()
 def stats(z):
