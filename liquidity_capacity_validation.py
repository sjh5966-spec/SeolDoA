...
px=con.execute(f"""SELECT symbol,TRY_CAST(report_date AS DATE) d,"open"::DOUBLE AS px_open,"close"::DOUBLE AS px_close,{volcol}::DOUBLE AS volume
...
for _,e in events.iterrows():
    ...
    if len(g): g['dollar_volume']=g.px_close*g.volume
    rows.append({**e.to_dict(),'pre20_n':len(g),
      ...
      'entry_dollar_volume':float(ent.iloc[0].px_close*ent.iloc[0].volume) if len(ent) else np.nan,
