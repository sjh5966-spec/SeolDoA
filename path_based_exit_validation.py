...
def trailing_after_trigger(g, trigger=0.20, trail=0.15):
    ...
    for _,r in g.iterrows():
        ...
        if armed:
            ...
            if oret<=stop_level:
                return oret,int(r.day_num),'TRAIL_GAP'
            if lret<=stop_level:
                return stop_level,int(r.day_num),'TRAIL'
    r=g.iloc[-1]
...
def no_response_exit(g, day_cut=10, min_ret=0.0):
    ...
    dcut=g[g.day_num==day_cut]
    if len(dcut):
        r=dcut.iloc[-1]; ret=r.px_close/ep-1
        if ret<=min_ret:
            return ret,int(r.day_num),'NO_RESPONSE'
    r=g.iloc[-1]
    return r.px_close/ep-1,int(r.day_num),'TIME'

def combo(g, trigger=0.20, trail=0.15, day_cut=10, min_ret=0.0):
    g=g.sort_values('day_num'); ep=float(g.entry_price.iloc[0]); armed=False; peak=None
    for _,r in g.iterrows():
        oret=r.px_open/ep-1; hret=r.px_high/ep-1; lret=r.px_low/ep-1
        if not armed:
        ...
        if armed:
            ...
            if oret<=stop_level: return oret,int(r.day_num),'TRAIL_GAP'
            if lret<=stop_level: return stop_level,int(r.day_num),'TRAIL'
        if int(r.day_num)==day_cut and not armed:
            ret=r.px_close/ep-1
            if ret<=min_ret: return ret,int(r.day_num),'NO_RESPONSE'
    r=g.iloc[-1]; return r.px_close/ep-1,int(r.day_num),'TIME'
...
for era,g in list(path.groupby('era'))+[('ALL',path)]:
    last=g.sort_values(['event_key','day_num']).groupby('event_key').tail(1).copy()
    x=last.px_close/last.entry_price-1
    base.append({'era':era,'rule':'R20_TIME','n':len(x),'mean':x.mean(),'median':x.median(),'win_rate':(x>0).mean(),'up20_rate':(x>=0.20).mean(),'down20_rate':(x<=-0.20).mean(),'mean_exit_day':last.day_num.mean()})
