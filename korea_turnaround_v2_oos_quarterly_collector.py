...
Fiscal years 2015..2025 are collected for extended PIT robustness; 2015 is prior context for 2016 signals. FY2025 Q4 is excluded because its disclosure
...
src = src.replace("HISTORICAL_SEED=Path('korea_dart_historical_seed_2015_2020.csv')", "HISTORICAL_SEED=Path('korea_turnaround_v2_oos_seed_2015_2025.csv')")
...
src = src.replace(
    ...
    "if year<2015 or year>2025: raise SystemExit('V2 extended PIT wrapper supports 2015..2025 only')\n    if year==2025: REPORTS[:] = [x for x in REPORTS if x[0] != 'Q4']"
)
