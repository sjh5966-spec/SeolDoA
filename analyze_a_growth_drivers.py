...
con=duckdb.connect('growth.duckdb')
...
SELECT *, rev_yoy-rev_yoy_prev AS rev_accel,
       fcf-fcf_yoy AS fcf_yoy_change,
       fcf-fcf_prev AS fcf_qoq_change,
       cash-cash_yoy AS cash_yoy_change,
       debt-debt_yoy AS debt_yoy_change,
       interest_expense-interest_yoy AS interest_yoy_change
FROM x
""")
...
FROM read_parquet('{F}')
WHERE form_type IN ('10-Q','10-K') AND TRY_CAST(report_date AS DATE) IS NOT NULL AND TRY_CAST(filing_date AS DATE) IS NOT NULL
GROUP BY 1,2
""")
con.execute(f"""
...
for feat in features:
    ...
    SELECT feature,quartile,COUNT(*) n,MIN(value) min_value,MAX(value) max_value,
           MEDIAN(value) median_feature,MEDIAN(r60d) median_r60d,AVG(r60d) mean_r60d,
           AVG(CASE WHEN r60d>0 THEN 1.0 ELSE 0.0 END) win_rate
    FROM z GROUP BY 1,2
    """)
...
SELECT
 CASE WHEN rev_accel>0 THEN 1 ELSE 0 END rev_accel_pos,
 CASE WHEN ebit_margin_yoy_change>0 THEN 1 ELSE 0 END margin_expand,
 CASE WHEN fcf_yoy_change>0 THEN 1 ELSE 0 END fcf_improve,
 CASE WHEN net_cash>0 THEN 1 ELSE 0 END net_cash_pos,
...
FROM a_events
WHERE rev_accel IS NOT NULL AND ebit_margin_yoy_change IS NOT NULL AND fcf_yoy_change IS NOT NULL AND net_cash IS NOT NULL
GROUP BY 1,2,3,4
""")
...
print('TOP COMBOS')
for r in con.execute('SELECT * FROM combos WHERE n_r60>=10 ORDER BY median_r60d DESC LIMIT 12').fetchall(): print(r)
