...
# event_key from prior common-axis build is stable inside this artifact.
syms=ev['symbol'].dropna().unique().tolist()
...
path=con.execute("""SELECT e.event_key,e.symbol,e.era,e.entry_price,p.rn-e.entry_rn_common day,p.d,p.px_open,p.px_high,p.px_low,p.px_close
...
agg=path.groupby(['event_key','symbol','era','entry_price'],as_index=False).agg(
    mfe20=('high_ret','max'), mae20=('low_ret','min'), r20_close=('close_ret','last'), n_days=('day','count'))
# day first crosses each threshold.
for th in [0.10,0.20,0.30,0.50]:
    first=path[path.high_ret>=th].groupby('event_key')['day'].min()
    agg[f'first_up_{int(th*100)}d']=agg.event_key.map(first)
for th in [0.10,0.15,0.20]:
    first=path[path.low_ret<=-th].groupby('event_key')['day'].min()
    agg[f'first_down_{int(th*100)}d']=agg.event_key.map(first)

# Conservative barrier simulation: if TP and SL are both touched on the same day, SL is assumed first.
def simulate_one(g,tp,sl):
    ep=float(g.entry_price.iloc[0])
    for _,r in g.sort_values('day').iterrows():
        # Gap at the open is executable at the open, not at an unavailable threshold price.
        oret=r.px_open/ep-1
        if oret<=-sl: return oret,int(r.day),'SL_gap'
        if oret>=tp: return oret,int(r.day),'TP_gap'
        hit_sl=(r.px_low/ep-1)<=-sl
        hit_tp=(r.px_high/ep-1)>=tp
        if hit_sl: return -sl,int(r.day),'SL'  # conservative same-day ordering
        if hit_tp: return tp,int(r.day),'TP'
    r=g.sort_values('day').iloc[-1]
...
for tp,sl in rules:
    ...
    for key,g in path.groupby('event_key'):
        ret,day,reason=simulate_one(g,tp,sl); vals.append((key,ret,day,reason))
...
# Baseline 20-day time exit on exactly the same events/path.
base_rows=[]
...
# Winner/loser path diagnostics by final R20 outcome.
diag=[]
